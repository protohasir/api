...
func TestIsUserMember(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "returns nil when user is a member",
			setupMock: func(m *MockMemberRoleChecker) {
				m.EXPECT().
					GetMemberRole(ctx, "org-1", "user-1").
		...
		{
			name: "returns nil when user is an author",
			setupMock: func(m *MockMemberRoleChecker) {
				m.EXPECT().
					GetMemberRole(ctx, "org-1", "user-1").
					Return(MemberRoleAuthor, nil)
		...
		{
			name: "returns nil when user is an owner",
			setupMock: func(m *MockMemberRoleChecker) {
				m.EXPECT().
					GetMemberRole(ctx, "org-1", "user-1").
					Return(MemberRoleOwner, nil)
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := IsUserMember(ctx, mockChecker, "org-1", "user-1")
